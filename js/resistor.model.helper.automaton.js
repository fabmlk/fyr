...
define(["jquery", "helper.colorset"], function ($, ColorSet) {
    ...
    return function () {
        ...
        this.getForbiddenColors = function (position) {
            var prop, res = [], set = currentWorkingSet[state], colors,
                filterFunc = function (elt) { // declaring outside a loop is jsLint-friendly
                    return Object.keys(set).indexOf(elt) !== -1;
                };
            ...
            for (prop in set) {
                if (set.hasOwnProperty(prop)) {
                    if (set[prop].indexOf(position) !== -1) {
                        colors = prop;
                        // replace wildcard with remaining colors
                        if (prop === '*') {
                            colors = ColorSet.all().filter(filterFunc);
                        }
                        res.concat(colors);
                    }
                }
